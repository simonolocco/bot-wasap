...
async function run() {
  ...
  check('precio exacto de fuente validada', () => {
    ...
    assert.match(response.text, /1\.234,56/); assert.match(response.text, /por kg/); assert.equal(response.outcome, 'answered');
  });
  ...
  check('no mezcla listas ni marcas', () => {
    ...
    assert.equal(renderAnswer({ ...base, productQuery: 'cremoso' }, catalog).outcome, 'clarify');
  ...
  const imported = await importExcel(await fs.readFile('data/productos.xlsx'), 'historico.xlsx');
