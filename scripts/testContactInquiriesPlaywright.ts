  ...
  try {
    for (const width of [390, 1440]) {
      ...
      await page.route('**/api/contacts?*', route => {
        ...
        return route.fulfill({json:{items:[{id:'qa',name:'Sergio',phone:'5491100000001',pipelineStatus:'new',labels:[],inquiryTypes: filtered ? ['lista_precio'] : ['horarios','direccion','lista_precio','hacer_pedido','asesor','preguntas_frecuentes','no_reconocidas']}],total:1,page:0,limit:25}});
      });
      ...
      await page.getByText('7 · No reconocidas',{exact:true}).waitFor();
      await page.getByLabel('Tipo de consulta',{exact:true}).selectOption('lista_precio');
      await page.getByText('3 · Precios',{exact:true}).waitFor();
