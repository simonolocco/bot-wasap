...
printf '%s\n' "UPDATE backup_runs SET status='succeeded', completed_at=now(), object_key=:'key', metadata=jsonb_build_object('tool','wal-g') WHERE id=:'run_id'" \
  | psql "$DATABASE_URL" -v run_id="$run_id" -v key="$WALG_S3_PREFIX" >/dev/null
